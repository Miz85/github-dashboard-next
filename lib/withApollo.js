 ...
 */
function initApolloClient(req, initialState) {
  ...
  if (!apolloClient) {
    apolloClient = createApolloClient(req, initialState);
  }
 ...
 * @param  {Object} [initialState={}]
 */
function createApolloClient(req = undefined, initialState = {}) {
  let host;
  let protocol;
  if (typeof window === 'undefined' && req && req.headers) {
    host = req
  ...
  const httpLinkUri =
    host && protocol
      ? `${protocol}://${host}${GRAPHQL_ENDPOINT_URL}`
